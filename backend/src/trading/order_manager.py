...
class OrderManager:
    ...
    def buy(self, symbol: str, amount_usdt: float, current_price: float,
            atr: Optional[float] = None, bucket: str = "day") -> Optional[dict]:
        coid = uuid.uuid4().hex
        self._log_create(coid, symbol, "buy", requested_amount=amount_usdt,
                         requested_price=current_price)

        if self.is_live:
            return self._buy_live(coid, symbol, amount_usdt, current_price, atr, bucket)
        return self._buy_paper(coid, symbol, amount_usdt, current_price, atr, bucket)

    def _buy_paper(self, coid, symbol, amount_usdt, current_price,
                   atr=None, bucket="day") -> Optional[dict]:
        pos = self._paper.place_market_buy(symbol, amount_usdt, current_price,
                                           atr=atr, bucket=bucket)
        if pos is None:
