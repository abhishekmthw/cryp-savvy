...
class TelegramAlerter:
    def __init__(self, bot_token: str = "", chat_id: str = "", *, capital_usdt: float = 0.0,
                 daily_loss_limit_usdt: float = 0.0):
        self._token = bot_token
        self._chat  = chat_id
        self._capital = capital_usdt
        self._daily_loss_limit = daily_loss_limit_usdt
    ...
    def alert_buy(self, symbol: str, price: float, amount_usdt: float,
                  score: float, mode: str = "paper"):
        tag = "[PAPER]" if mode == "paper" else "[LIVE]"
        self._send(
            f"*{tag} BUY* `{symbol}`\n"
            f"Price: ${price:,.4f}\n"
            f"Amount: ${amount_usdt:,.2f}\n"
